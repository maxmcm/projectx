...
###    -->  docker run -d -v //c/test/://home/seluser/Downloads -p 4445:4444 -p 5901:5900 selenium/standalone-firefox-debug:2.53.0
...
for (i in url_rand[]) {
  ...
  if (file.exists(file.path(trks, df_urls[i,5], paste(df_urls[i,3],"_", df_urls[i,2], "_", df_urls[i,1], ".txt", sep = "", collapse = ""))) == FALSE) { 
    ...
    fprof <- makeFirefoxProfile(list(browser.startup.homepage = "about:blank"
                                     ...
                                     , browser.helperApps.neverAsk.saveToDisk = "application/pdf, application/octet-stream, application/x-pdf, application/acrobat, applications/vnd.pdf, text/pdf, text/x-pdf, application/vnd.cups-pdf"

...
for (m in 20:20) {
    ...
    for (r in 1:nraces) {
      ...
      # Parsing text
      trk_row <- 1; track_fn(parse_race, trk_row) ## TRACK
      can_row <- trk_row + 1; canceled_fn(parse_race, can_row, trk_tmp) ## CANCELLATION
      rtype_row <- trk_row + 1; racetype_fn(parse_race, rtype_row) ## RACE TYPE - STILL TO DO: race type additional info (eg: arabian stakes)  ##grep(('^.*CLAIMING|^.*STAKES|^.*ALLOWANCE|^.*SPEED INDEX RACE|^.*SPECIAL WEIGHT|^.*MAIDEN'),parse_race)
      ttype_row <- grep(('^.*Track Record:|^.*On The'),parse_race); tracktype_fn(parse_race, ttype_row) ## TRACK TYPE - distance etc - STILL TO DO: when trackrecordtime is blank, take care of "originally scheduled for"
      racecriteria_fn(rtype_row, ttype_row, parse_race) ## RACE CRITERIA
      prs_row <- grep("^Purse:",parse_race); purse_fn(parse_race, prs_row) ## PURSE - STILL TO DO: deal with "check for anything else on this row"Added", "Guaranteed"
      avm_row <- grep("^Available Money:",parse_race); availmoney_fn(parse_race, avm_row) ## AVAILABLE MONEY
      wea_row <- grep("^Weather:",parse_race); weather_fn(parse_race, wea_row) ## WEATHER
      valueofrace_fn(parse_race, avm_row, wea_row) ## VALUE OF RACE
      off_row <- grep("^Off at:", parse_race); offat_fn(parse_race, off_row) ## OFF AT
      ft_row <- grep("^Fractional Times:", parse_race); fractimes_fn(parse_race, ft_row) ## FRACTIONAL TIMES
      fint_row <- grep("*Final Time:", parse_race); finaltime_fn(parse_race, fint_row) ## FINAL TIME
      st_row <- grep("^Split Times:", parse_race); splittimes_fn(parse_race, st_row) ## SPLIT TIMES
      ru_row <- grep("^Run-Up:", parse_race); runup_fn(parse_race, ru_row) ## RUN UP
      win_row <- grep("^Winner: ", parse_race); winner_fn(parse_race, win_row) ## WINNER
      breed_row <- grep("^Breeder: ", parse_race); breeder_fn(parse_race, breed_row) ## BREEDER
      scratch_row <- grep("^Scratched Horse", parse_race); scratching_fn(parse_race, scratch_row) ## SCRATCHINGS - still to do amend for no scratchings
...
#      colnames(df_pprlrow) <- c("fst_pos", "fst_lth", "snd_pos", "snd_lth", "thd_pos", "thd_lth", "fth_pos", "fth_lth", "str_pos", "str_lth", "fin_pos", "fin_lth" )
  ...
  #      ntp <- length(test)
    ...
    #  }
      ...
      df_subraces[r,] <- bind_rows(data.frame(trk_tmp, can_tmp, rtype_tmp, rc_tmp, ttype_tmp, prs_tmp, avm_tmp, wea_tmp, vor_tmp, off_tmp, ft_tmp, fint_tmp, st_tmp, ru_tmp, win_tmp, breed_tmp, scratch_tmp,  wpspl_tmp, first_tmp, second_tmp, third_tmp, exact_tmp, trif_tmp, super_tmp, train_tmp, owners_tmp, foot_tmp))
